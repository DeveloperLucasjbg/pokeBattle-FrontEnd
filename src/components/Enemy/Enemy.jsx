...
import './Enemy.css';
  ...
  export default function Enemy () {
    ...
    const { enemy,win, colecction } = useContext(BattleContext)
    let classCondition = win === null ? 'Player1' : win === 'player' ? 'Player1-lose' : 'Player1-win';

    useEffect(() => {
      setPoke(findPokeById(enemy,colecction))
    }, [colecction,enemy])
     

    return (
      <div className={classCondition}>
        <img className='Player1-image' src={poke?.imageUrl ? poke?.imageUrl : pokeball  } alt={poke?.name} />
